...
class BasisHamburgerBtn {
  constructor(args = {}) {
    this.args = jquery.extend({
      btn: '.c-hamburger-btn'
    }, args);
    this.hamburgerBtn = jquery(this.args.btn);
    this.setListener();
  }

  setListener() {
    this.hamburgerBtn.each((i, e) => {
      const hamburgerBtn = jquery(e);
      const target = jquery('#' + hamburgerBtn.attr('aria-controls'));
...
class BasisDrawer {
  constructor(args = {}) {
    ...
    this.drawer = jquery(this.args.drawer);
    this.setListener();
  }

  setListener() {
    this.drawer.each((i, e) => {
      const drawer = jquery(e);
      this.setIdForSubmenu(drawer);

      const container  = drawer.parent();
      const btn        = jquery(`#${drawer.attr('aria-labelledby')}`);
      ...
      toggleBtns.each((i, e) => {
        const toggleBtn = jquery(e);
        const submenu   = jquery(`#${toggleBtn.attr('aria-controls')}`);
  ...
  toggleMenu(btn) {
    const menu = jquery(`#${btn.attr('aria-controls')}`);
...
class BasisNavbar {
  constructor(args = {}) {
    ...
    this.items = jquery(`${this.args.item}[aria-haspopup="true"], ${this.args.subitem}[aria-haspopup="true"]`);
    this.setListener();
  }
  ...
  setListener() {
    ...
    jquery(this.args.item).each((i, e) => {
      const item = jquery(e);
...
class BasisPageEffect {
  constructor(args = {}) {
    this.args = jquery.extend({
      pageEffect: '.c-page-effect',
      duration: 200
    }, args);
    this.container     = jquery(this.args.pageEffect);
    this.pageOutObject = jquery('[data-page-effect-link="true"], a[href]:not([target="_blank"], [href^="#"], [href*="javascript"], [href*=".jpg"], [href*=".jpeg"], [href*=".gif"], [href*=".png"], [href*=".mov"], [href*=".swf"], [href*=".mp4"], [href*=".flv"], [href*=".avi"], [href*=".mp3"], [href*=".pdf"], [href*=".zip"], [href^="mailto:"], [data-page-effect-link="false"])');
    this.setListener();
  }
...
class BasisSelect {
  constructor(args = {}) {
    ...
    this.select = jquery(this.args.select);
    this.select.each((i, e) => {
      const selectWrapper = jquery(e);
      ...
      select.on('change', (event) => {
        label.text(jquery(select[0].selectedOptions).text());
      });
...
/**
 * Created by tumugu2 on 2016/12/14.
 */
const hamburgerBtn = new BasisHamburgerBtn();

const drawer = new BasisDrawer();

const navbar = new BasisNavbar();

const pageEffect = new BasisPageEffect();

const select = new BasisSelect();

export { hamburgerBtn, drawer, navbar, pageEffect, select };
